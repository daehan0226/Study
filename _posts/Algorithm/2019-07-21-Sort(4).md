...
2. 힙 정렬
  1. 가장 큰 값(인덱스:0)을 가장 밑(마지막 인덱스(n-1))값과 교환
  2. 각 부모1,자식2 그룹에서 먼저 자식 중 큰 값을 왼쪽(더 작은 인덱스 값으로) 바꾼다. 결국 
  3. 자식중 더 큰 값이 부모보다 크면 부모와 자식(왼쪽)과 교환 -> 큰 값이 부모가 되고 그 부모는 결국 2.1로 돌아가 인덱스(n-2)와 교환
